...
**Answer** Nevada had the largest percentage growth.
...
**Answer** Howard County, Maryland has the highest median household income estimate. However, they have a MOE of 2,169. But, Calvert County, Maryland, which has the second highest median household income estimate, has a MOE of 3,736. Hence, it could be either or. 
...
**Answer** No, because the bars overlap and the two counties are within the margin of error, so the differences aren't significant. 

Is the difference between Howard and everyone else significant?
**Answer** Yes, because there aren't any bars that overlap it, and its population estimate is higher compared to others. 
...
**Answer** Somerset County and Caroline County, Maryland both saw their median incomes fall from 2010-2019 negatively. The changes are outside the margin of error.
...
**Answer** 21202, 21740 and 21201.

```{r}
md_zipcodes <- get_acs(geography="zcta", variables = "B19013_001", state='MD')
md_zips_with_acs <- top_zips %>% left_join(md_zipcodes, by=c('zip5'='GEOID')) %>% arrange(desc(total))
