...
**Answer** Data files that are titled Hospital. They come with different file extensions, like .cpg, .dbf, .prj ("which indicates that projection is specified"), .shp ("the main file that pulls it all together"), .shx, and .xml. I believe they have hospital bed information in each. 
...
**Answer** The columns in this data include, ObjectID (I believe that's just 1-7,581), ID (which I think is a very specific unique number for each hospital), address, city, state, zip, "zip4" (idk what that is), a phone number (which the first few don't have), type (of hospital it is), status (possibly whether the hospital still operates? Maybe specifically during COVID?), population, county (of the hospital), the county (most likely only US), Latitude and Logitute of the hospital, the NAICS code and it's description, and a bunch of other columns to describe each hospital.
...
**Answer** The MD hospitals are taken from the data set, and it shows the state's hospitals and their information through the columns. The geometry column specifically, something I didn't notice before, took the longitude and latitude information and put it into points. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The type of MD hospitals was grouped together and summarized. It shows that we have General Acute Care (50, the most), Psychiatric (11), Military (4), Chronic Disease (2), Long Term Care (2), Rehabilitation (2), and Special hospitals (1). 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** General Acute Care hospitals from Maryland have been taken out, and we can see the information of each of the 50 hopsitals individually. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Each hospital has been visually represented to a point on what looks like a scatterplot. But it's not really a scatterplot? The longitude and latitude are also the x and y axis. Looking closely, it also looks like the shape of Maryland. So it's essentially a map in the making. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** It says: "To enable caching of data, set 'options(tigris_use_cache = TRUE)' in your R script or .Rprofile. Not sure as to what this means. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** It shows the various columns of the "county" database, like the statefp (which, I assume is the state's fips code?), the countyfp, GEOID, name and so on. 
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We took out the Maryland counties from the counties database through the statefp, which is equal to 24. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We see a full on map of Maryland. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We've combined the map we just created and the data/points of the hospitals together.  
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Now it looks pretty. We've added color, along with a color key (I think that's what it's called) to show the how many beds are in each pointed hospital. 
...
**Answer** Just running the libraries. It does say my API key already exists, but that's frequent.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** It shows the 50 General Acute Care hospitals in Maryland and the information about each. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** It shows the total amount of hospital beds in each county. Baltimore City County has the most amount (4,505), and Kent has the lowest (26).
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The information from tidycensus and code below shows the population of residents living in each county in Maryland. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** What I'm seeing is essentially the same as what is above, however, it now includes a geometry column, which looks like longtitude and latitude values for some counties. Other counties have a block-looking symbol in it.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** We see both the dataframes, the dataframe that includes the population and longtitude and latitude points of each county, and the dataframe that includes the total amount of beds for each county. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Looks like the county, it's population and the geometry (longitude and latitude values), are in one dataframe. (Question: why does it come up as a symbol in the dataframe on its own, but comes up as numbers below? Queen Anne's shows up as a symbol in the dataframe, but below I see a number.)
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The county, population, and geometry is shown in this dataset, along with the total amount of beds from the "md_beds_by_county dataframe. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** The values for Queen Anne's County and Carolina County now have zeros instead of NA. This allows us to do the math. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** This dataframe has a new column, which is beds_per_100k, which is the total_bed divided by the population, multiplied by 100,000 (how many beds per that amount of people). 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Ooooh. We're taking the database shown and putting it into a map. The color also shows how many beds are in each county, per 100k people.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** Same thing, now just with different colors, which is cute. 
...
**Answer** The South of Maryland has the fewest amount of beds per 100k. The west side of Maryland has more beds per 100k than the middle of Maryland. 
