...
**Answer** The dataframe shows all of the U.S. states, the 2016 presidential candidate that won such state and the percentage of votes, the percent of small businesses in each state that received a PPP loan, and the average amount of money from the PPP loan given to each small business employee in such state. (Question: why does the amount per employee matter? Where was Reveal going with that field?)   
```{r}
...
**Answer** The points that are towards the right of the scatter plot have a higher percentage vote for Trump and a higher percentage of small businesses recieving PPP loans. Based on the dataframe from earlier, if this was in datawrapper where we could see the points and what state they represent, they would include Nebraska, North Dakota, Oklahoma, and so on. The points on the left of the scatter plot, which is also lower than the points on the right, I assume would represent the states that didn't receive many PPP loans and also voted less for Trump (hence, more for Clinton -- further hence, bluer states). 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** This one looks a little all over the place. So it seems like there isn't much of a correlation between pct_trump and ppp_amount_per_employee. 
(Question: does having the ppp_amount_per_employee, help the reporters "check their work?")
```{r}
...
**Answer** The dataframe below shows the correlation between the percentage of businesses that received PPP loans and Trump's percentage of votes in each state, and the average amount from the PPP loans given to each small business employee and Trump's percentage of votes in each state. There's a stronger correlation of the latter, but not really between the amount per employee and the percentage of votes for Trump in each state. 
```{r}
reveal_data %>%
...
**Answer** The p-value is 8.607e-05, which is 0.00008607, which is also very low. It also shows the number from before, 0.5218, which was the correlation between the businesses receiving the PPP loan to the percentages of votes Trump got from each state. 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** The p-value is higher now: 0.1189. It shows the same number of 0.2211 from earlier. 
```{r}
...
**Answer** The dataframe shows the averages of the amount per employee and the percentage of businesses that received PPP loans.  There is a higher amount, for Trump, which additionally proves that there is a difference. 
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The t-test in this is equal to -1.2223. The p-value is also 0.2295
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The t-value in this is now -3.182 and the p-value is 0.002643, which I would believe shows that there's something more meaningful in the finding.
```{r}
