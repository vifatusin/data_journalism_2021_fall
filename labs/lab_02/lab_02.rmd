...
wv_ppp_loan_amount_categories <- west_virginia_ppp %>%
  select(amount) %>% 
  mutate(case_when(amount < 10000 ~ "under_10k", 
                   amount >= 10000 & 50000 ~ "10k_50k",
                   amount >= 50000 & 100000 ~ "50k_100k",
                   amount >= 100000 & 500000 ~ "100k_500k", 
                   amount >= 500000 & 1000000 ~ "500k_1m",
                   amount >= 1000000 ~ "1m_plus")) %>% 
  mutate(under_10k = amount < 10000) %>% 
  mutate(between_10k_50k = amount >= 10000 & 50000) %>% 
  mutate(btw_50k_100k = amount >= 50000 & 100000) %>% 
  mutate(btw_100k_500k = amount >= 100000 & 500000) %>% 
  mutate(btw_500k_1m = amount >= 500000 & 1000000) %>% 
  mutate(more_than_1m = amount >= 1000000) %>%
  arrange(amount)
